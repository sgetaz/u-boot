...
static int is_valid_hw_id(u8 variant)
{
	...
	if (variant < ARRAY_SIZE(ccwmx53_id)) {
		if (ccwmx53_id[variant].mem[0] != 0)
			ret = 1;
	}
	return ret;
}
...
{
	return ccwmx53_id[variant].mem[0];
}

static unsigned int get_cpu_freq_from_hwid(u8 variant)
{
	return ccwmx53_id[variant].cpu_freq;
}
...
int get_module_hw_id(void)
{
	...
	memset(&mod_hwid, 0, sizeof(struct ccwmx53_hwid));
...
#ifdef CONFIG_CCWMX53
	gd->bd->bi_arch_number = MACH_TYPE_CCWMX53JS;
#elif CONFIG_CCMX53
	gd->bd->bi_arch_number = MACH_TYPE_CCMX53JS;
#endif
...
void print_sys_info(void)
{
	...
	if (boot_verb) {
		...
		if (is_valid_hw_id(mod_hwid.variant)) {
			puts("Module HW ID  : ");
			printf("%s (%02x)\n", ccwmx53_id[mod_hwid.variant].id_string, mod_hwid.variant);
